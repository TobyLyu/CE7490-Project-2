...
class GaloisField:
    def __init__(self, w) -> None:
        ...
        if self.w == 4:
            ...
            self.gflog = np.zeros(self.max_mask, dtype=np.uint8)
            self.gfilog = np.zeros(self.max_mask, dtype=np.uint8)
        elif self.w == 16:
            # x^16 + x^12 + x^3 + x + 1
            self.prim_poly = 0b10001000000001011
            self.gflog = np.zeros(self.max_mask, dtype=np.uint16)
            self.gfilog = np.zeros(self.max_mask, dtype=np.uint16)
        self.gen_tables()
    ...
    def matrix_inverse(self, i_a): 
        ...
        # TODO: check inverse consistency: m_a * m_i = I
        return m_i   
